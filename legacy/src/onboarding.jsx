...
function OnboardingFlow({ onComplete, onBack, lang, prefs, setPrefs }) {
  ...
  const canContinue = () => {
    ...
    if (step === 2) {
      // require at least 1 taste per selected cat (overall >= 2)
      return prefs.tastes.length >= 2;
    }
    return true;
  };
    ...
    <div className="onb">
      ...
      <div className="onb-footer">
        <button
          ...
          {step === TOTAL ? t('finish') : t('continue')} <window.Icon name="arrowRight" size={16} />
        </button>
      </div>
    </div>
  );
...
function Step1Categories({ prefs, setPrefs, lang }) {
  ...
  const toggle = (id) => {
    const next = prefs.cats.includes(id)
      ...
      : [...prefs.cats, id];
    // Also remove tastes that belonged to this cat if unselected
    let nextTastes = prefs.tastes;
    if (!next.includes(id)) {
      const catTasteIds = (window.TASTES[id] || []).map(t => t.id);
        ...
        <div className="cat-grid">
          {window.CATEGORIES.map(c => (
            <div
              ...
              <div className="cat-check"><window.Icon name="check" size={14} stroke={3} /></div>
            </div>
          ))}
        </div>
      </div>
      <div className="onb-counter">
        {prefs.cats.length}/{window.CATEGORIES.length} {lang === 'es' ? 'seleccionadas' : 'selected'}
      </div>
    </>
  );
}
...
function Step2Tastes({ prefs, setPrefs, lang }) {
  ...
  const selectedCats = window.CATEGORIES.filter(c => prefs.cats.includes(c.id));
  return (
    <>
      <div className="onb-header">
      ...
      <div className="onb-body">
        {selectedCats.map(cat => {
          const tastes = window.TASTES[cat.id] || [];
          ...
          <div className="radio-row">
            {locs.map(l => (
              <div
                ...
                <window.Icon name="mapPin" size={18} />
                <div>
...
function Step4Ready({ prefs, lang }) {
  ...
  const matching = window.DEALS.filter(d =>
      ...
      <div className="onb-body" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', marginTop: 18, maxWidth: 280 }}>
          {prefs.tastes.slice(0, 8).map(tid => {
            const taste = Object.values(window.TASTES).flat().find(t => t.id === tid);
